...
class Network(nn.Module):
    def __init__(self, input_dim, output_dim, r):
        ...
        self.r = r
        self.param = nn.Parameter(torch.randn(r, input_dim, output_dim))

    def forward(self, t):
        k = int(self.r * t)
        k = max(0, min(k, self.r - 1))
        return self.param[k]
...
class ODEFunc_order_1(nn.Module):
    def __init__(self, p, r, input_dim):
        super(ODEFunc_order_1, self).__init__()
        self.p = p
        self.r = r
        self.input_dim = input_dim

        self.w_net = Network(1, self.p * self.input_dim, r)
        self.a_net = Network(1, self.p * self.input_dim, r)
        self.b_net = Network(1, self.p, r)
...
class FullModel_order_1(nn.Module):
    def __init__(self, input_dim = 2, p = 16, r = 20):
        super(FullModel_order_1, self).__init__()
        self.ode_func = ODEFunc_order_1(p, r, input_dim)
...
class FullModel_order_2(nn.Module):
    def __init__(self, input_dim = 2, p = 16, r = 20):
        ...
        self.input_dim = input_dim
        self.p = p
        self.ode_func = ODEFunc_order_2(p, r, input_dim)
...
def get_model(p, input_dim, r, order = 1):
    if order == 1:
        return FullModel_order_1(input_dim, p, r)
    else:
        return FullModel_order_2(input_dim, p, r)
...
def plot_result(X_train, X_test, y_test, model, pred, path, count, run = None):
    ...
    plt.savefig(path + str(count) + ("_" + str(run) if run is not None else "") + '_decision_boundary' + '.png')
    ...
    plt.savefig(path + str(count) + ("_" + str(run) if run is not None else "") + '_predictions' + '.png')
